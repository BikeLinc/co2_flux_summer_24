...
grid on;
legend([h2(1), h3(1)], ["Burn Site", "Control Site"]);
yregion(range, 'FaceColor',"magenta",'FaceAlpha', 0.1, 'DisplayName', 'Expected Range');
yline(0.4, 'k--',"DisplayName", "Extrapolation Boundary");
...
licorBurnLumpHour = rmmissing(licorBurnLumpHour);
licorUnburnLumpHour = rmmissing(licorUnburnLumpHour);
daqBurnLumpHour = rmmissing(daqBurnLumpHour);
daqUnburnLumpHour = rmmissing(daqUnburnLumpHour);
% Convert datetime to numeric using datenum
x_unburn = datenum(daqUnburnLumpHour.T);
y_unburn = daqUnburnLumpHour.F_mg * 86400;
e_unburn = daqUnburnLumpHour.uF_mg * 86400;

x_burn = datenum(daqBurnLumpHour.T);
y_burn = daqBurnLumpHour.F_mg * 86400;
e_burn = daqBurnLumpHour.uF_mg * 86400;

figure();
hold on;
grid on;

% Plot error bars for Unburned DAQ
errorbar(x_unburn, y_unburn, e_unburn, 'r');

% Plot error bars for Burned DAQ
errorbar(x_burn, y_burn, e_burn, 'b');

% Format the x-axis to display datetime labels
datetick('x', 'keeplimits', 'keepticks');

xlabel("Time");
ylabel("Flux Rate [mg/m^2/day]");
title("Hourly Measured CO_2 Flux Rates");
legend("Dynamic Un-Burn", "Dynamic Burn");
yregion(range*86400, 'FaceColor', "magenta", 'FaceAlpha', 0.1, 'DisplayName', 'Expected Range');
